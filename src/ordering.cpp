...
std::vector<Index> minimum_degree_ordering(const CscMatrix& a) {
    const Graph g = Graph::from_matrix(a);
    const Index n = g.n;
    // adj[i]: sorted list of non-eliminated neighbours of i in the current
    // elimination graph.  Eliminating v makes its neighbours a clique.
...
// The elimination graph adjacency of i is  A_i  U  (union of L_e, e in E_i),
// but it is never formed.  Eliminating p:
//   L_p = (A_p U union_{e in E_p} L_e) \ {p}
//   every e in E_p is absorbed into p (deleted)
//   for i in L_p:  A_i := A_i \ L_p \ {p},  E_i := (E_i \ E_p) U {p}
// and the *approximate external degree* of i in L_p is
//   d(i) = min( n - k - nv(i),
//               d_old(i) + |L_p \ i|,
//               |A_i| + |L_p \ i| + sum_{e in E_i, e != p} |L_e \ L_p| )
// where |L_e \ L_p| is obtained for all e at once by the w(e) counting trick.
// Indistinguishable variables (same A_i and E_i) are merged into
// supervariables with weight nv; all set sizes are weighted by nv.
...
std::vector<Index> amd_ordering(const CscMatrix& a) {
    ...
    std::vector<Index> mark(sz(n), 0);   // membership stamp for L_p
    Index stamp = 0;
    ...
    Index k = 0;         // weighted number of eliminated variables
    Index mindeg = 0;
    while (k < n) {
        ...
        // ---- form L_p = (A_p U union L_e, e in E_p) \ {p} ------------------
        ++stamp;
        mark[sz(p)] = stamp;
        Lp.clear();
        Index lp_weight = 0;
        ...
        for (Index e : E[sz(p)]) {
            ...
            alive[sz(e)] = 0;                 // element absorption
            std::vector<Index>().swap(L[sz(e)]);
        }
        std::vector<Index>().swap(A[sz(p)]);
        std::vector<Index>().swap(E[sz(p)]);

        // p becomes an element; output p and everything merged into it
        status[sz(p)] = ELEMENT;
        alive[sz(p)] = 1;
        ...
        for (Index i : Lp) {
            ...
            // E_i := (E_i \ absorbed) U {p};  deg_e = sum |L_e \ L_p|
            Index deg_e = 0;
            Index h = 0;
            std::size_t out = 0;
            ...
            // A_i := A_i \ L_p \ {p} \ merged variables;  a_weight = |A_i|
            Index a_weight = 0;
            out = 0;
            ...
            const Index ext_lp = lp_weight - nv[sz(i)];   // |L_p \ i|
        ...
        // ---- supervariable detection: same hash, identical A_i and E_i ------
        for (Index i : Lp) {
        ...
        for (Index i : Lp) {
            const Index h = hash[sz(i)];
            if (hhead[sz(h)] == -1) continue;    // bucket already processed
            // compare every pair in this bucket
            for (Index x = hhead[sz(h)]; x != -1; x = hnext[sz(x)]) {
                ...
                for (Index y = hnext[sz(x)]; y != -1; y = hnext[sz(y)]) {
                    ...
                    // y is indistinguishable from x: merge y into x
                    degree[sz(x)] -= nv[sz(y)];
                    nv[sz(x)] += nv[sz(y)];
                    nv[sz(y)] = 0;
                    status[sz(y)] = MERGED;
