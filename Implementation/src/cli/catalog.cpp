...
// reading it fully is the validation: a matrix the tool can't parse never enters the catalog
...
bool Catalog::insert(const CatalogEntry& e, std::string& err, int* out_id) {
    ...
    // upsert: keeps the existing id on a re-import, so mtx_<id> never changes
    sqlite3_stmt* st = nullptr;
    if (sqlite3_prepare_v2(db_,
    ...
    // SQLITE_STATIC: the strings outlive the statement, no copy needed
