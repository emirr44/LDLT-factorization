...
struct Session {
    ...
    FactorResult last;                 // most recent factorization, for `save`
};
...
// Also drops a UTF-8 byte-order mark, which Windows tools put in front of
// text piped or redirected into a program.
...
// "import some file.mtx" -> "import", "some file.mtx"
...
// Paths dropped into a terminal arrive quoted ("C:\a b\x.mtx") or, on macOS
// and Linux, with escaped spaces (/a\ b/x.mtx). Undo both.
...
// Text padded to an on-screen width (std::setw counts bytes, not characters).
...
// Relative to the current directory when that is shorter, else absolute.
...
void factorize(Session& session, const std::string& path, const std::string& label) {
    ...
    if (r.threw) return;   // unreadable file: keep the previous result for `save`
