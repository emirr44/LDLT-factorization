...
CscMatrix CscMatrix::from_coo(const CooMatrix& coo) {
    ...
    a.sort_columns();   // also sums duplicates
    return a;
}
...
CscMatrix CscMatrix::transpose() const {
    // Classic counting-sort transpose (cs_transpose).  Output columns are
    // sorted by construction because we walk the input columns in order.
    const Index nz = nnz();
