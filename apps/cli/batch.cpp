// batch.cpp - the non-interactive commands, for scripts and measurements.
//
//   ldlt info    <file.mtx>                 sparsity statistics
//   ldlt print   <file.mtx> [limit]         dense print (n <= limit, default 20)
//   ldlt convert <in.mtx> <out.mtx> [--symmetric]
//   ldlt factor  <file.mtx>... [options]    LDL' factorization + residual check
//        --order <m>      natural (default) | rcm | md | amd
//        --postorder      compose the ordering with the etree postorder
//        --print          print L and D (automatic when n <= 12)
//        --check-factor   also compute ||PAP' - LDL'||_F / ||A||_F (n <= 3000)
//        --ones           use x_true = ones instead of pseudo-random
//        --tol <t>        treat |d| <= t as a zero pivot (default 0)
//        --static-pivot <r>  perturb pivots with |d| < r*||A|| to +-r*||A|| (e.g. 1e-8)
//        --refine <k>     up to k steps of iterative refinement after the solve
//   With several files, factor prints one table row per matrix.
//   ldlt orderings <file.mtx>...             nnz(L), flops and time for every ordering
//
// Running `ldlt` with no arguments starts the interactive session instead.
...
struct FactorRun {
    ...
    double t_read = 0.0, t_order = 0.0, t_sym = 0.0, t_num = 0.0, t_solve = 0.0;   // seconds
    bool ok = false;
    Index failed_pivot = -1, neg = 0, perturbed = 0;
    Real dmin = 0.0, dmax = 0.0;
    Real backward = 0.0, forward = 0.0, recon = -1.0;
    Real backward0 = 0.0;      // before refinement
