...
$flags = @("-std=c++17", "-Wall", "-Wextra", "-Wpedantic", "-Wshadow", "-Wconversion",
           "-I$root\include", "-I$root\third_party\sqlite", "-I$gen")
if ($Debug) { $flags += @("-g", "-O0", "-D_GLIBCXX_ASSERTIONS") }
else        { $flags += @("-O2", "-DNDEBUG") }

$lib = Get-ChildItem (Join-Path $root "src\*.cpp")      | ForEach-Object { $_.FullName }
$cli = Get-ChildItem (Join-Path $root "apps\cli\*.cpp") | ForEach-Object { $_.FullName }
