...
void init(bool force_plain) {
    // NO_COLOR and FORCE_COLOR are the usual conventions; FORCE_COLOR keeps
    // colour when piping into a pager such as `less -R`.
...
#ifdef _WIN32
    if (g_fancy && tty) {
        // Windows consoles need to be told to understand UTF-8 and ANSI
        // colour codes. A console too old to accept that gets plain output.
...
std::size_t width(const std::string& s) {
    ...
    for (std::size_t i = 0; i < s.size(); ++i) {
        ...
        if (c == 0x1b) {                       // skip "ESC [ ... m"
            while (i < s.size() && s[i] != 'm') ++i;
            continue;
        }
        if ((c & 0xC0) != 0x80) ++w;           // count UTF-8 lead bytes only
    }
    return w;
}
